...
void build_symbols::push_scope(scope* s){
    std::cout << "BUILD: entering scope: " << s << std::endl;
    this -> scope_stack.push(s);
}
void build_symbols::pop_scope(){
    std::cout << "BUILD: leaving scope: " << this -> get_current_scope() << std::endl;
...
void build_symbols::visit_class_type(class_type *n){
    ...
    if(c == NULL){
        std::cout << "ERROR: could not resolve class symbol" << std::endl;
    }
    else{
        n -> set_symbol(c);
        std::cout << "SUCCESS: resolved class type: " << n -> get_symbol() -> get_name() <<  std::endl;
    }
    visit_children(n);
}
...
void build_symbols::visit_block_node(block_node *n) { 
    std::cout << "BUILD: building block symbol" << std::endl;
    this -> push_scope(n);
    visit_children(n);
    this -> pop_scope();
}
...
void build_symbols::visit_name_exp(name_exp *n) {
    ...
    if(n -> get_is_lhs()){
        std::cout << "BUILD: resolving simple name: " << n -> get_name() << std::endl;
        ...
        std::cout << "BUILD: resolving LHS of name: " << n -> get_name() << std::endl;
        expression* lhs_exp = n -> get_expression();


        //RESOLVE EXPRESSION
        visit_ast(lhs_exp);
        std::cout << "BUILD: LHS: " << lhs_exp -> get_type() -> get_name() << std::endl; 
        ...
        if(n -> get_is_call()){
            std::cout << "BUILD: visiting RHS of call" << std::endl;
            class_sym = lhs_scope -> find_method(n -> get_name());
        }
        else{
            std::cout << "BUILD: visiting RHS of field" << std::endl;   
            class_sym = lhs_scope -> find_scope(n -> get_name());
        }
    }    
    
    if(class_sym != NULL){
        std::cout << "SUCCESS: resolved symbol: " << class_sym -> get_name() << std::endl;
